...
class Encoding:
    def __init__(
        ...
        *,
        ...
        self._core_bpe = _parrotnlp.CoreBPE(mergeable_ranks, special_tokens, pat_str)
    ...
    def encode_to_numpy(
        ...
        *,
        ...
        buffer = self._core_bpe.encode_to_parrotnlp_buffer(text, allowed_special)
    ...
    def __getstate__(self) -> object:
        import parrotnlp.registry

        # As an optimisation, pickle registered encodings by reference
        if self is parrotnlp.registry.ENCODINGS.get(self.name):
            return self.name
        return {
            "name": self.name,
    ...
    def __setstate__(self, value: object) -> None:
        ...
        if isinstance(value, str):
            self.__dict__ = parrotnlp.registry.get_encoding(value).__dict__
            return
        self.__init__(**value)
