...
def read_file_cached(blobpath: str, expected_hash: str | None = None) -> bytes:
    ...
    if "parrotnlp_CACHE_DIR" in os.environ:
        cache_dir = os.environ["parrotnlp_CACHE_DIR"]
...
def data_gym_to_mergeable_bpe_ranks(
    ...
    vocab_bpe_contents = read_file_cached(
        vocab_bpe_file, vocab_bpe_hash).decode()
    ...
    # this sanity check is important since parrotnlp assumes that ranks are ordered the same
    # as merge priority
    encoder_json = json.loads(read_file_cached(
        encoder_json_file, encoder_json_hash))
...
def dump_parrotnlp_bpe(bpe_ranks: dict[bytes, int], parrotnlp_bpe_file: str) -> None:
    try:
        import blobfile
    except ImportError as e:
        raise ImportError(
    ...
    with blobfile.BlobFile(parrotnlp_bpe_file, "wb") as f:
        for token, rank in sorted(bpe_ranks.items(), key=lambda x: x[1]):
            f.write(base64.b64encode(token) +
                    b" " + str(rank).encode() + b"\n")


def load_parrotnlp_bpe(parrotnlp_bpe_file: str, expected_hash: str | None = None) -> dict[bytes, int]:
    # NB: do not add caching to this function
    contents = read_file_cached(parrotnlp_bpe_file, expected_hash)
    ret = {}
    for line in contents.splitlines():
        ...
        try:
            ...
            raise ValueError(
                f"Error parsing line {line!r} in {parrotnlp_bpe_file}") from e
    return ret
