...
def redact(dry_run: bool) -> None:
    parrotnlp_root = Path(__file__).parent.parent
    assert parrotnlp_root.name == "parrotnlp"
    assert (parrotnlp_root / "pyproject.toml").exists()

    try:
        output = subprocess.check_output(["git", "ls-files"], cwd=parrotnlp_root, text=True)
        paths = [Path(p) for p in output.splitlines()]
    except subprocess.CalledProcessError:
        paths = list(parrotnlp_root.glob("**/*"))
