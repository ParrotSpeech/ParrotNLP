...
def test_basic_encode():
    enc = parrotnlp.get_encoding("r50k_base")
    assert enc.encode("hello world") == [31373, 995]

    enc = parrotnlp.get_encoding("p50k_base")
    assert enc.encode("hello world") == [31373, 995]
...
def test_hyp_encode_bytes(make_enc: Callable[[], parrotnlp.Encoding], bytestring: bytes):
    enc = make_enc()
    assert enc.decode_bytes(enc._encode_bytes(bytestring)) == bytestring
...
@pytest.mark.parametrize("make_enc", ENCODING_FACTORIES)
def test_catastrophically_repetitive(make_enc: Callable[[], parrotnlp.Encoding]):
    enc = make_enc()
    for c in ["^", "0", "a", "'s", " ", "\n"]:
        big_value = c * 10_000
...
@hypothesis.settings(deadline=None, max_examples=MAX_EXAMPLES)
def test_hyp_roundtrip(make_enc: Callable[[], parrotnlp.Encoding], text):
    enc = make_enc()
...
@pytest.mark.parametrize("make_enc", ENCODING_FACTORIES)
def test_single_token_roundtrip(make_enc: Callable[[], parrotnlp.Encoding]):
    enc = make_enc()
...
@pytest.mark.parametrize("make_enc", ENCODING_FACTORIES)
def test_batch_encode(make_enc: Callable[[], parrotnlp.Encoding]):
    enc = make_enc()
    text1 = "hello world"
    text2 = "goodbye world"
...
@hypothesis.settings(deadline=None, max_examples=MAX_EXAMPLES)
def test_hyp_batch_roundtrip(make_enc: Callable[[], parrotnlp.Encoding], batch):
    enc = make_enc()
